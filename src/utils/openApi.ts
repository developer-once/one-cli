...
export const prettierFile = (content: string, options?: prettier.Options): [string, boolean] => {
  let result = content;
  let hasError = false;
  try {
...
export const writeFile = (folderPath: string, fileName: string, content: string, prettierOptions?: prettier.Options) => {
  const filePath = path.join(folderPath, fileName);
  mkdir(path.dirname(filePath));
  const [prettierContent, hasError] = prettierFile(content, prettierOptions);
