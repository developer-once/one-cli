...
class ServiceGenerator {
  ...
  constructor(config: GenerateServiceProps, openAPIData: OpenAPIObject) {
    ...
    // 白名单更新 controller 文件
    if (this.config.increment?.includes && this.config.increment?.includes?.length) {
      const tags = this.config.increment.includes.map((item) => resolveTypeName(item));
      this.apiData = pick(this.apiData, tags);
    }

    // if (this.config.increment?.excludes && this.config.increment?.excludes?.length && !this.config.increment?.includes) {
    //   const tags = this.config.increment?.excludes.map((item) => resolveTypeName(item));
    //   this.apiData = omit(this.apiData, tags);
    // }
  }

  public genFile() {
    const basePath = this.config.serversPath || './src/service';
    let previousSwagger: OpenAPIObject;
    ...
    try {
      if (this.config.increment) previousSwagger = require(resolve(this.finalPath, 'swagger.json'));
    } catch (error) {
      Log('🈲 未能获取旧 swagger.json 数据，将不进行增量操作');
      // throw error;
    }

    if (this.config.increment?.types && previousSwagger) {
      // 启用 类型增量
      // 如果当前目录下保留了 上一次的 swagger.json，读取并进行增量
      try {
        const previousSwaggerTypes = previousSwagger.components?.schemas;
        const currentSwaggerTypes = this.openAPIData.components?.schemas;

        // 找不同
        const diffKeys = this.getIncrementDiffKeys(
          Object.keys(currentSwaggerTypes),
          Object.keys(previousSwaggerTypes),
        );

        // 将旧types重新填充到新types
        const oldDiffTypes = {};
        diffKeys.forEach((key) => {
          oldDiffTypes[key] = previousSwaggerTypes[key];
        });
    ...
    if (this.config.increment) {
      // 先使用 tags 解析
      if (previousSwagger) {
        const previousTags = previousSwagger.tags.map((i) => this.replaceDot(i.name));
        const currentTags = Object.keys(this.apiData);

        const diffKeys = this.getIncrementDiffKeys(currentTags, previousTags);
        this.classNameList = this.classNameList
          .concat(diffKeys.map((name) => ({ fileName: name, controllerName: name })))
  ...
  public getInterfaceTP() {
    ...
    Object.keys(this.openAPIData.paths || {}).forEach((p) => {
      ...
      ['get', 'put', 'post', 'delete', 'patch'].forEach((method) => {
        ...
        if (operationObject.parameters) {
          ...
          // fix: when params in path and not match parameters from swagger docs
          let parameters = operationObject.parameters
          if (method === 'get') {
            const { query = [], path = [] } = this.getParamsTP(operationObject.parameters, p)
        ...
        if (props.length > 0 && data) {
          const typeName = `${this.getFuncationName({
            ...operationObject,
            method,
            path: p,
          })}Params`;
    ...
    // 自定义swagger refs 类型
    if (typeof this.config.hook?.customRefType === 'function') {
      parsedData = parsedData.map((i) => {
        const customProps = this.config.hook.customRefType({
          props: i.props?.[0],
          // 提供原 schema 名称 schemaName，typeName 为修饰后的名称
          // 因GET方法入参由上一段代码自动修饰，没有对应的schemaName，typeName，
          // 即GET方法 typeName === schemaName
          schemaName: i.schemaName,
          typeName: i.typeName,
        }) || [];

        return { ...i, props: [customProps] };
      });
    }
  ...
  private genOpenApiData() {
    try {
      return writeFile(this.finalPath, 'swagger.json', JSON.stringify(this.openAPIData), {
        ...this.config.prettierOptions,
        parser: 'json',
      });
    } catch (error) {
      Log('🛑 生成swagger.json失败');
      throw error;
    }
  }

  private getIncrementDiffKeys(currentKeys: string[], previousKeys: string[]) {
    return (xor(previousKeys, currentKeys) as string[]).filter((key) => !currentKeys.includes(key));
  }
